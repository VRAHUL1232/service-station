...
function ScratchCard() {
  ...
  useEffect(() => {
    ...
    const setupRealtimeListeners = () => {
      ...
      const unsubscribeLeftScratch = onValue(leftScratchRef, (snapshot) => {
      ...
      const unsubscribeLeftDent = onValue(leftDentRef, (snapshot) => {
      ...
      const unsubscribeRigthScratch = onValue(rightScratchRef, (snapshot) => {
      ...
      const unsubscribeRightDent = onValue(rightDentRef, (snapshot) => {
      ...
      const unsubscribeTopDent = onValue(topDentRef, (snapshot) => {
      ...
      const unsubscribeTopScratch = onValue(topScratchRef, (snapshot) => {
  ...
  return (
    <div className="w-full h-full items-center p-6 bg-[#f3eded]">
  <h1 className="flex text-3xl text-black justify-center font-bold">
    Scratch & Dent Analysis
  </h1>
  <div className="flex flex-row items-center justify-center">
    <div className="flex flex-col items-center w-auto min-h-[100px]">
  ...
  <div className="flex flex-row justify-between font-bold text-xl mt-4">
    <div className="flex flex-row gap-2 justify-center">
      ...
      <h4 className="text-[#a100ff] text-xl">
        <span className="text-[#460073]">{leftDent+rightDent+topDent}</span> Dent
      </h4>
      <h4 className="text-[#0041f0] text-xl">
        <span className="text-[#460073]">{leftScratch+rightScratch+topScratch}</span> Scratch
      </h4>
    </div>
    <div className="flex flex-col">
